...
const uploadProductImagesToS3 = async (
	...
	if (thumbnail?.file && thumbnail.file !== initialThumbnail) {
		try {
			const thumbnailConfig = await getProductUploadUrl(
				thumbnail.file.type,
				folder
			);
			...
			toast.error(
				"Unable to get upload URL for thumbnail. Please check permissions."
			);
			return null;
		}
	}
	...
	for (let i = 0; i < images.length; i++) {
		...
		if (
			img.file &&
			(img.file !== initialImages[i]?.file || !initialImages[i])
		) {
			...
			try {
				...
				console.error(
					`Failed to get upload URL for image: ${img.file.name}`,
					error
				);
				toast.error(
					`Unable to get upload URL for image: ${img.file.name}. Please check permissions.`
				);
				return null;
			}
		}
	...
	try {
		...
		toast.error(
			"An unexpected error occurred. Deleted previously uploaded images."
		);
		return null;
	}
};
